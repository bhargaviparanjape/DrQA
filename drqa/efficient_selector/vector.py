...
def vectorize(ex, model, single_answer=False):
    ...
    if 'gold_label' not in ex:
        return document, features, question, ex['id']

    # ...or with target(s) (might still be empty if answers is empty)
    # if single_answer:
    #     assert(len(ex['answers']) > 0)
    #     gold_id = torch.zeros(len(sentences))
    #     for e, gold_sentence in enumerate(ex['gold_sentence_ids']):
    #         gold_id[e] = 1.0
    # else:
    gold_id = torch.LongTensor(1).fill_(ex['gold_label'])
    return document, features, question, gold_id, ex['id']
...
def batchify(batch):
    ...
    if len(batch[0]) == NUM_INPUTS + NUM_EXTRA:
        ...
        if torch.is_tensor(batch[0][3]):
            y = torch.cat([ex[3] for ex in batch])
    else:
        raise RuntimeError('Incorrect number of inputs per example.')

    return x1, x1_f, x1_mask, x2, x2_mask, y, ids
