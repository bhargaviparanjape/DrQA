...
def vectorize(ex, model, single_answer=False):
    ...
    for sent in ex['sentences']:
        sentence_boundaries.append([counter, counter + len(sent)])
        counter += len(sent)
    sentence_indices = torch.LongTensor(ex['sentence_indices'])
    ...
    if single_answer:
        ...
        ## Multiple Golds are still not supported
    ...
    return document, features, question, sentence_indices, selected_offset, start, end, gold_id, ex['id']
...
def batchify(batch):
    ...
    sentence_indices = [ex[3] for ex in batch] #padding required
    offsets = [ex[4] for ex in batch]
    ...
    # Batch sentence indices
    sentence_lengths = [d.size(0) for d in sentence_indices]
    max_sentences = max(sentence_lengths)
    x1_idx = torch.LongTensor(len(sentence_indices), max_sentences, 2).zero_()
    x1_idxmask = torch.ByteTensor(len(sentence_indices), max_sentences).fill_(1)
    for i, d in enumerate(sentence_indices):
        x1_idx[i, :d.size(0), :].copy_(d)
        x1_idx[i, d.size(0):, 1].fill_(1) # Fill in 0,1 in the masked sentences to prevent empty listing
        x1_idxmask[i, :d.size(0)].fill_(0)
    ...
    if len(batch[0]) == NUM_INPUTS + NUM_EXTRA:
        ...
        if torch.is_tensor(batch[0][5]):
            ...
            y_sp = torch.cat([ex[7] for ex in batch])
        else:
            y_s = [ex[5] for ex in batch]
            y_e = [ex[6] for ex in batch]
            y_sp = [ex[7] for ex in batch]
    else:
        raise RuntimeError('Incorrect number of inputs per example.')

    return x1, x1_f, x1_mask, x1_idx, x1_idxmask, x2, x2_mask, y_s, y_e, y_sp, offsets, ids
