...
class DocReader(object):
    ...
    def update(self, ex):
        ...
        if self.use_cuda:
            ...
            target_sp = Variable(ex[9].cuda(async=True))
        else:
    ...
    def predict(self, ex, candidates=None, top_n=1, async_pool=None):
        ...
        if candidates:
            ...
            args = (score_s, score_e, score_sp, top_n, self.args.max_len)
            if async_pool:
    ...
    @staticmethod
    def decode(score_s, score_e, score_sp, top_n=1, max_len=None):
        ...
        for i in range(score_s.size(0)):
            ...
            sp_scores = score_sp[i].numpy()
            sp_scores_flat = sp_scores.flatten()
            if top_n == 1:
                idx_sort = [np.argmax(sp_scores_flat)]
            elif len(sp_scores_flat) <= top_n:
                idx_sort = np.argsort(-sp_scores_flat)
            else:
                idx = np.argpartition(-sp_scores_flat, top_n)[0:top_n]
                idx_sort = idx[np.argsort(-sp_scores_flat[idx])]
            pred_sp.append(idx_sort)
        return pred_s, pred_e, pred_score, pred_sp

    @staticmethod
    def decode_candidates(score_s, score_e, score_sp, candidates, top_n=1, max_len=None):
        ## TODO: Support for support prediction
