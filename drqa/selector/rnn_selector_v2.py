...
class RnnSentSelector2(nn.Module):
    RNN_TYPES = {'lstm': nn.LSTM, 'gru': nn.GRU, 'rnn': nn.RNN}

    def __init__(self, args, normalize=True):
        super(RnnSentSelector2, self).__init__()
        # Store config
        self.args = args
    ...
    def forward(self, x1, x1_f, x1_mask, x1_sent_mask, x2, x2_mask):
        ...
        x1_emb_flattened = x1_emb.view(x1_emb.shape[0], x1_emb.shape[1]*x1_emb.shape[2], x1_emb.shape[3])
        ...
        x1_mask_flattened = x1_mask.view(x1_mask.shape[0], -1)
        ...
        if self.args.use_qemb:
            x2_weighted_emb = self.qemb_match(x1_emb_flattened, x2_emb, x2_mask)
            drnn_input.append(x2_weighted_emb)

        # Add manual features
        if self.args.num_features > 0:
            x1_f_flattened =  x1_f.view(x1_f.shape[0], x1_f.shape[1]*x1_f.shape[2], x1_f.shape[3])
            drnn_input.append(x1_emb_flattened)
		...
		## reshape doc_hidden and now expand qhidden
