...
def fix_span(para, offsets, span):
	...
	if isinstance(offsets[0][0], list):
		begins, ends = map(list, zip(*[y for x in offsets for y in x]))
	else:
		begins, ends = map(list, zip(*offsets))
...
def _process_article(article, config):
	...
	for para in paragraphs:
		...
		# sentence splitting already done for paragraph
...
def nlp_pipeline(sent):
	doc = nlp(sent)
	return doc

def check_is_support(sentence_boundaries, answers):
	gold_sentence_ids = []
	for s_idx, tup in enumerate(sentence_boundaries):
		for a in answers:
	...
	return gold_sentence_ids

def _process_article_squadlike(article, config):
	# paragraph has context and list of qas
	paragraph =  article[0]
	title = article[1]
	...
	# Context
	context_nlp = nlp_pipeline(context)
	context_tokens = [token.text for token in context_nlp]
	context_chars = [list(token) for token in context_tokens]
	context_spans = convert_idx(context, context_tokens)
	# context_spans = [[N_chars + e[0], N_chars + e[1]] for e in context_spans] # being called only once
	flat_offsets = context_spans

	# Questions and Answers
	examples = []
	eval_examples = []
	for qap in paragraph['qas']:
		question_text = qap['question']
		ques_tokens = word_tokenize(prepro_sent(question_text))
		ques_chars = [list(token) for token in ques_tokens]
		if 'answers' in qap:
			answers = qap['answers']
			answer_texts = []
			for ans in answers:
				...
				if ans_text not in ''.join(context):
					best_indices_list.append((0, 1))
				else:
					best_indices_list = []
					_, best_indices, _ = fix_span(context, flat_offsets, ans_text)
					answer_span = []
					for idx, span in enumerate(flat_offsets):
						if not (best_indices[1] <= span[0] or best_indices[0] >= span[1]):
							answer_span.append(idx)
			...
			best_indices = list(set(best_indices_list))
		else:
			answer = 'random'
			best_indices = [(0, 1)]
		...
		sentence_boundaries = []
		sent2title_ids = []
		sent2title_ids.append((title, -1))
		count = 0
		for s_idx, sent in enumerate(context_nlp.sents):
			sentence_boundaries.append((count, count + len([token for token in sent])))
			count += len([token for token in sent])
			sent2title_ids.append((title, s_idx))

		gold_sentence_ids = check_is_support(sentence_boundaries, best_indices)
		for s_idx, sent_ in enumerate(sentence_boundaries):
			if s_idx in gold_sentence_ids:
				start_end_facts.append((sent_[0], sent_[1], 1))
			else:
				start_end_facts.append((sent_[0], sent_[1], 0))

		example = {'context_tokens': context_tokens, 'context_chars': context_chars, 'ques_tokens': ques_tokens,
		           'ques_chars': ques_chars, 'y1s': [idx[0] for idx in best_indices],
		           'y2s': [idx[1] for idx in  best_indices], 'id': qap['id'],
		           'start_end_facts': start_end_facts}
		eval_example = {'context': context, 'spans': flat_offsets, 'answer': answer_texts, 'id': qap['id'],
		                'sent2title_ids': sent2title_ids}
		examples.append((example, eval_example))
...
def process_file(filename, config, word_counter=None, char_counter=None):
	...
	if config.dataset == "squad":
		...
		for article in articles:
			...
			for p_idx, para in enumerate(paragraphs):
				data.append((para, article['title'] + "_" + str(p_idx))) # context; list of qas
	...
	if config.dataset == "squad":
		outputs = Parallel(n_jobs=12, verbose=10)(delayed(_process_article_squadlike)(article, config) for article in data)
