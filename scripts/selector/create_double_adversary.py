...
def load_dump_dataset(path):
    ...
    for idx, article in enumerate(data):
        ...
        for i, paragraph in enumerate(article['paragraphs']):
            ...
            output['contexts'].append(paragraph["context"] + " " + paragraph["context"])
            for qa in paragraph['qas']:
                ...
                if 'answers' in qa:
                    ...
                    for ans in qa["answers"]:
                        new_answer = {}
                        new_answer["answer_start"] = ans["answer_start"] + len(paragraph["context"]) + 1 #Space
                        new_answer["text"] = ans["text"]
                        new_answers.append(new_answer)
                    combined_answers = qa["answers"] + new_answers
                    output['answers'].append(combined_answers)
            data[idx]["paragraphs"][i]["context"] = paragraph["context"] + " " + paragraph["context"]
    output_file = path.split(".", 1)[0] + "-doubled" + ".json"
...
def process_dataset(data, tokenizer, workers=None):
    ...
    workers = make_pool(initargs=(tokenizer_class, {'annotators': {'lemma'}, 'classpath' : "/home/ubuntu/bvp/DrQA/data/corenlp/*"}))
    ...
    workers = make_pool(
	initargs=(tokenizer_class, {'annotators': {'lemma', 'pos', 'ner'},'classpath' : "/home/ubuntu/bvp/DrQA/data/corenlp/*"})
    ...
    for idx in range(len(data['qids'])):
        ...
        if len(data['answers']) > 0:
            for ans in data['answers'][idx]:
                # Change the definition in find_answers to return all offsets
...
if __name__ == '__main__':
    ...
    dataset = load_dump_dataset(in_file)

    out_file = os.path.join(
        args.out_dir, '%s-doubled-processed-%s.txt' % (args.split, args.tokenizer)
    )
